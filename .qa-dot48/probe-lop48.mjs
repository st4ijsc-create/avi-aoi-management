...
//   node .qa-dot48/probe-lop48.mjs [--base=http://localhost:3048] [--vp=1600x900,1280x720] [--tag=lop]
import { chromium } from "@playwright/test";
import { existsSync, mkdirSync, readFileSync, writeFileSync } from "node:fs";
const arg = (k, d) => { const m = process.argv.find((a) => a.startsWith(`--${k}=`)); return m ? m.slice(k.length + 3) : d; };
const BASE = arg("base", "http://localhost:3048"); const VPS = arg("vp", "1600x900,1280x720").split(","); const TAG = arg("tag", "lop");
const OUT = ".qa-dot48/lop"; mkdirSync(OUT, { recursive: true });
