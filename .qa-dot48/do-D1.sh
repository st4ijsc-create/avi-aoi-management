...
echo "--- andon raised TAM may 14 → a2,a3,a4 @1600 ---"; node .qa-dot37/db.mjs andon-tam tao 14 | tee .qa-dot48/raised-tao.json | tr -d '\n'; echo
node .qa-dot44/do32.mjs --role=A --vp=1600x900 --cases=a2,a3,a4 --tag=$T/raised-1600x900 --base=$B > .qa-dot48/raised-node.log 2>&1; echo "node exit=$?"; grep -E "$L" .qa-dot48/raised-node.log
node .qa-dot37/db.mjs andon-tam xoa | tee .qa-dot48/raised-xoa.json | tr -d '\n'; echo
