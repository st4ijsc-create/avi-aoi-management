...
//   node .qa-dot48/nguon-khung.mjs --man=may|twin|line [--base=http://localhost:3048] [--vp=1600x900]
// Ghi .qa-dot48/nguon-khung/<man>-<vp>.json + in bảng: khung ← rAF(stack) ← commit(fiber/prop) ← mạng. KHÔNG sửa mã.
import { chromium } from "@playwright/test";
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
const arg = (k, d) => { const m = process.argv.find((a) => a.startsWith(`--${k}=`)); return m ? m.slice(k.length + 3) : d; };
const MAN = arg("man", "may"); const BASE = arg("base", "http://localhost:3048");
const [VW, VH] = arg("vp", "1600x900").split("x").map(Number); const VP = `${VW}x${VH}`;
const OUT = ".qa-dot48/nguon-khung"; mkdirSync(OUT, { recursive: true });
...
await ctx.addCookies(JSON.parse(readFileSync(".qa-dot48/state-e2e_tai_loE.json", "utf8")));
