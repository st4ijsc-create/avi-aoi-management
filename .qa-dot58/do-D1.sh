#!/bin/sh
# ĐỢT 58 · D-1 — 48 ca Đợt 32 NGUYÊN TRẠNG (harness ĐÓNG BĂNG .qa-dot44/do32.mjs, KHÔNG sửa một dòng)
# chạy trên cổng 3058, TUẦN TỰ (G97). Hàng tạm có trap (G111).  sh .qa-dot58/do-D1.sh <nhan>
...
echo "=== [D-1 $N] bat dau $(date -Iseconds) ==="; node .qa-dot37/db.mjs dem
...
echo "--- andon raised TAM may 14 -> a2,a3,a4 @1600 ---"; node .qa-dot37/db.mjs andon-tam tao 14 | tee .qa-dot58/d1$N-raised-tao.json | tr -d '\n'; echo
node .qa-dot44/do32.mjs --role=A --vp=1600x900 --cases=a2,a3,a4 --tag=$T/d1$N-raised-1600x900 --base=$B > .qa-dot58/d1$N-raised-node.log 2>&1; echo "node exit=$?"; grep -E "$L" .qa-dot58/d1$N-raised-node.log
node .qa-dot37/db.mjs andon-tam xoa | tee .qa-dot58/d1$N-raised-xoa.json | tr -d '\n'; echo
echo "=== [D-1 $N] xong $(date -Iseconds) ==="
