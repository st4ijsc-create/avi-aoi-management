...
// ★ Đợt 49 — cột 8 nay = XÂY Đợt 49 (.qa-dot49/nen-*); cột 7 giữ HEAD Đợt 47. Luật 48 ca GIỮ NGUYÊN.
// ★ Đợt 48 — cột 7 = HEAD Đợt 47 (.qa-dot47/nen-*); cột 8 = QA lần 7 Đợt 48 (.qa-dot48/nen-*). Luật 48 ca GIỮ NGUYÊN.
const D47 = { A: ".qa-dot47/nen-1600x900", A12: ".qa-dot47/nen-1280x720", R: ".qa-dot47/raised-1600x900" };
const MOI = { A: ".qa-dot49/nen-1600x900", A12: ".qa-dot49/nen-1280x720", R: ".qa-dot49/raised-1600x900" };
const rows = [];
...
if (process.argv.includes("--md=1")) {
  console.log("| # | Ca | Đợt 32 | | Đợt 37 | | Đợt 39 | | Đợt 41 | | Đợt 44 | | Đợt 46 | | Đợt 47 | | XÂY Đợt 49 | | Ghi chú |\n|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|");
...
console.log(`\nN=${rows.length} · ĐỢT 32 ${JSON.stringify(dem("pcu"))} · ĐỢT 37 ${JSON.stringify(dem("pd37"))} · ĐỢT 39 ${JSON.stringify(dem("pd39"))} · ĐỢT 41 ${JSON.stringify(dem("pd41"))} · ĐỢT 44 ${JSON.stringify(dem("pd44"))} · ĐỢT 46 ${JSON.stringify(dem("pd46"))} · ĐỢT 47 ${JSON.stringify(dem("pd47"))} · XÂY ĐỢT 49 ${JSON.stringify(dem("pmoi"))}`);
const doi = rows.filter((r) => r.pd47 !== r.pmoi); console.log(`Đổi phán quyết Đợt 47 → XÂY Đợt 49: ${doi.length}: ${doi.map((r) => `#${r.id} ${r.pd47}→${r.pmoi}`).join(", ") || "(không)"}`);
