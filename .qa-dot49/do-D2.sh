...
# Đầu ra .qa-dot49/$M/K-*, E, I, P (G65). Khuôn = .qa-dot37/do-D2.sh (QA Đợt 37), chỉ đổi cổng + thư mục. Hàng tạm có trap (G111).
cd /d/SOURCES/_twin_wt || exit 1
B=http://localhost:3049
M="${MANG:-chan}"; T=../.qa-dot49/$M
...
mkdir -p .qa-dot49/$M/E
node .qa-dot37/db.mjs chen-msl 14 > .qa-dot49/$M/E/e9-chen-msl.json; cat .qa-dot49/$M/E/e9-chen-msl.json; echo
MSL=$(node -e 'console.log(JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")).hang.id)' .qa-dot49/$M/E/e9-chen-msl.json)
node .qa-dot37/db.mjs msl-ts "$MSL" > .qa-dot49/$M/E/e9-msl-ts.json; cat .qa-dot49/$M/E/e9-msl-ts.json; echo
node .qa-dot35/db.mjs robot-ts > .qa-dot49/$M/E/e9-robot-db.json
node .qa-dot44/do35.mjs --role=A --vp=1600x900 --cases=e9 --tag=$T/E --base=$B --mang=$M 2>&1 | grep -E "$L"
node .qa-dot37/db.mjs xoa-msl "$MSL" > .qa-dot49/$M/E/e9-xoa-msl.json; cat .qa-dot49/$M/E/e9-xoa-msl.json; echo; MSL=""
...
echo "--- P3-P7 @1600 + @1280 (.qa-dot44/do38.mjs → .qa-dot49/$M/P) ---"
for VP in 1600x900 1280x720; do node .qa-dot44/do38.mjs --cases=p3,p4,p5,p6,p7 --vp=$VP --tag=../.qa-dot49/$M/P --base=$B --mang=$M 2>&1 | grep -vE '^\s*$'; done
