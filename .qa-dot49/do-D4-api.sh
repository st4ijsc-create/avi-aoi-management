...
for V in A B C D M ADM; do echo "--- tRPC vai $V ---"; node .qa-dot49/api-vai.mjs --vai=$V --base=$B; done
for V in A B C D M ADM; do echo "--- socket vai $V ---"; node .qa-dot49/socket-vai.mjs --vai=$V --base=$B --giay=12; done
echo "--- HTTP v1 (api key tam SIM-FAC) ---"; node .qa-dot41/apikey-tam.mjs tao > .qa-dot49/api-vai/apikey-tao.json; sed 's/"key":"[^"]*"/"key":"(an)"/' .qa-dot49/api-vai/apikey-tao.json; echo
KEY=$(node -e 'console.log(JSON.parse(require("fs").readFileSync(".qa-dot49/api-vai/apikey-tao.json","utf8")).key)')
node .qa-dot49/http-v1.mjs --key="$KEY" --base=$B
node .qa-dot41/apikey-tam.mjs xoa | tee .qa-dot49/api-vai/apikey-xoa.json; echo
sed -i 's/"key":"[^"]*"/"key":"(da xoa khoi tep tho)"/' .qa-dot49/api-vai/apikey-tao.json
