#!/bin/sh
# ĐỢT 48 · D-1 (mạng CDN bị chặn: mang.mjs) — 48 ca Đợt 32 NGUYÊN TRẠNG (harness .qa-dot44/do32.mjs) trên 3043, hai vp, vai A/B/C + andon raised TẠM. TUẦN TỰ (G97). Đầu ra .qa-dot44/nen-*/ (G65). Hàng tạm có trap (G111).
...
echo "--- andon raised TAM may 14 → a2,a3,a4 @1600 ---"; node .qa-dot37/db.mjs andon-tam tao 14 | tee .qa-dot49/raised-tao.json | tr -d '\n'; echo
node .qa-dot44/do32.mjs --role=A --vp=1600x900 --cases=a2,a3,a4 --tag=$T/raised-1600x900 --base=$B > .qa-dot49/raised-node.log 2>&1; echo "node exit=$?"; grep -E "$L" .qa-dot49/raised-node.log
node .qa-dot37/db.mjs andon-tam xoa | tee .qa-dot49/raised-xoa.json | tr -d '\n'; echo
