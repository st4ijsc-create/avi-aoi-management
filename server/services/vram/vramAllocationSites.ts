...
export const KNOWN_ALLOCATION_SITES: readonly {
  ...
  { file: "server/services/aiLocalTools/repoCommandSandbox.ts", symbol: "child_process", wired: false, note: "import spawn — bộ chạy lệnh của `run_command` (doc 78 PHA B + doc 79 D). Danh sách TRẮNG 9 mục: npm run check · npm run check:tests · npx vitest run <đường> · git status · git diff · dotnet build/test/format <đường> · node --test <đường>. ⚠ ĐÍNH CHÍNH (doc 82): dòng này từng khai '5 mục' — SAI từ lượt doc 79 D thêm bốn mục .NET/node. `dotnet format` (mục thứ TÁM, `ghiDia: true`) GHI ĐÈ tệp .cs — mục DUY NHẤT làm vậy — nhưng vẫn KHÔNG chạm GPU. KHÔNG mục nào chạm GPU, và không có shell nào trên đường đi." },
