...
function makeFakeDb() {
  return {
    ...
    update: () => ({
      set: (patch: Row) => ({
        // ★★★ 2026-08-23 — cùng khuôn thenable + `.returning()` như `insert().values()` ngay dưới:
        // `confirmAction` giành quyền bằng `UPDATE … WHERE status=<đã quan sát>` rồi ĐẾM hàng trả về.
        where: (pred: (r: Row) => boolean) => {
