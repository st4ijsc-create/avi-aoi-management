 ...
 *
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 * VÌ SAO `kind: "write"` KHI LỆNH NÀO CŨNG CHỈ ĐỌC
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 * Cả năm mục trong danh sách trắng đều là lệnh HỎI/KIỂM TRA — không mục nào ghi vào cây làm việc.
 * Vậy mà tool vẫn là `write`, và đó là một quyết định có tải trọng: `kind` ở repo này quyết định
...
function cauTuChoiLenh(ma: MaTuChoiLenh, chiTiet: string, lang: ToolLang): string {
  const bang = DANH_SACH_TRANG.map((m) => `• ${m.nhan} — ${m.moTa}`).join("\n");
  switch (ma) {
    case "CMD_METACHAR":
      return w(
        lang,
    ...
    case "CMD_NOT_ALLOWED":
      return w(
        lang,
        `Lệnh "${chiTiet}" KHÔNG nằm trong danh sách TRẮNG. Đây là danh sách trắng, không phải danh sách đen: mọi lệnh không được khai đều bị từ chối, và danh sách không nới ra lúc chạy. Năm lệnh được phép:\n${bang}\n⚠ git checkout / git reset / rm / DROP KHÔNG BAO GIỜ được thêm vào — ngày 2026-08-18 một tác nhân chạy "git checkout <file>" để hoàn nguyên một sửa đổi 1 dòng và xoá mất 123 dòng chưa commit.`,
        `Command "${chiTiet}" is NOT on the allowlist. This is an allowlist, not a denylist: anything not declared is refused, and the list never widens at runtime. The five allowed commands:\n${bang}\n⚠ git checkout / git reset / rm / DROP will never be added.`,
        `命令 "${chiTiet}" 不在白名单中。这是白名单而非黑名单：未声明的一律拒绝，且运行时不会放宽。允许的五条命令：\n${bang}\n⚠ git checkout / git reset / rm / DROP 永不加入。`,
      );
