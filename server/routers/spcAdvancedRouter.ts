...
function stdDev(values: number[], m: number, sample = true): number {
...
function createSubgroups(data: { value: number; inspectionTime: Date }[], size: number): SubgroupStats[] {
...
function computeControlLimits(subgroups: SubgroupStats[], size: number) {
...
function checkWesternElectricRules(values: number[], cl: number, sigma: number): RuleViolation[] {
...
function checkNelsonRules(values: number[], cl: number, sigma: number): RuleViolation[] {
  const violations: RuleViolation[] = [];
  const n = values.length;

  // Nelson Rule 1 = same as WE Rule 1 (beyond 3σ) - skip duplicate
  ...
  // Nelson Rule 5: 2 of 3 beyond 2σ same side (same as WE Rule 2 - skip)

  // Nelson Rule 6: 4 of 5 beyond 1σ same side (same as WE Rule 3 - skip)
  ...
  for (let i = 14; i < n; i++) {
    ...
    if (window.every(v => Math.abs(v - cl) < sigma)) {
      violations.push({
        ruleType: 'nelson_7', ruleName: 'Nelson Rule 7: 15 pts within 1σ (stratification)',
        ruleDescription: `15 consecutive points within 1σ at indices ${i - 14}-${i}. Possible stratification or incorrect subgrouping.`,
        severity: 'info', violatingIndices: Array.from({ length: 15 }, (_, k) => i - 14 + k), violatingValues: window,
      });
    }
  }

  // Nelson Rule 8: 8 consecutive beyond 1σ on EITHER side (mixture)
  for (let i = 7; i < n; i++) {
    const window = values.slice(i - 7, i + 1);
    if (window.every(v => Math.abs(v - cl) > sigma)) {
      violations.push({
        ruleType: 'nelson_8', ruleName: 'Nelson Rule 8: 8 pts beyond 1σ (mixture)',
        ruleDescription: `8 consecutive points beyond 1σ at indices ${i - 7}-${i}. Possible mixture of distributions.`,
        severity: 'warning', violatingIndices: Array.from({ length: 8 }, (_, k) => i - 7 + k), violatingValues: window,
      });
    }
  }
