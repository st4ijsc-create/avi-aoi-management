 ...
 * Accepts base64, data URL, or Buffer input.
 */
export function computeImageHash(input: string | Buffer): string {
  let buffer: Buffer;
  if (Buffer.isBuffer(input)) {
    ...
    buffer = Buffer.from(dataUrlMatch ? dataUrlMatch[1] : trimmed, "base64");
  }
