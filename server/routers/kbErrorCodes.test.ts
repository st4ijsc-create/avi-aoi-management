...
describe("mã lỗi luồng nạp tri thức — hàm dựng lỗi (kbErrors.ts)", () => {
  ...
  it("phân tích thất bại ⇒ KB_PARSE_FAILED kèm lý do nguyên văn", () => {
    const err = buildParseFailedError("corrupt pdf");
    const meta = readAppErrorMeta(err);
    expect(meta?.appCode).toBe("KB_PARSE_FAILED");
    expect(meta?.appParams).toEqual({ reason: "corrupt pdf" });
  });
  ...
  it("tải URL thất bại ⇒ KB_FETCH_FAILED kèm url và lý do", () => {
    const err = buildFetchFailedError("https://example.com/page", "timed out");
    const meta = readAppErrorMeta(err);
    expect(meta?.appCode).toBe("KB_FETCH_FAILED");
    expect(meta?.appParams).toEqual({ url: "https://example.com/page", reason: "timed out" });
  });
...
describe("kbIngestRouter — appCode thật sự tới được TRPCError.cause (không chỉ đúng ở kbErrors.ts)", () => {
  ...
  it("service ném KbParseError ⇒ KB_PARSE_FAILED kèm lý do nguyên văn", async () => {
  ...
  it("ingestUrl: service ném SsrfBlockedError ⇒ KB_FETCH_FAILED kèm url gốc + lý do, code tRPC vẫn BAD_REQUEST", async () => {
    ...
    const meta = readAppErrorMeta(err);
    expect(meta?.appCode).toBe("KB_FETCH_FAILED");
    expect(meta?.appParams).toEqual({ url: "https://example.com/page", reason: "blocked IP" });
  });
...
describe("kbStudioRouter — appCode thật sự tới được TRPCError.cause (mirror kbIngestRouter)", () => {
  ...
  it("ingestUrlJob: service ném FetchError ⇒ KB_FETCH_FAILED kèm url gốc, job vẫn được đánh dấu failed", async () => {
