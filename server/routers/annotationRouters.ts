  ...
  save: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (existing.rows?.length > 0) {
        // Update existing
        await db.execute(
          sql`UPDATE image_annotations SET annotations = ${JSON.stringify(input.annotations)}, "updatedAt" = NOW() WHERE id = ${existing.rows[0].id}`
        );
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      for (let i = 0; i < input.imageUrls.length; i++) {
        ...
        if (existing.rows?.length > 0) {
          ...
          await db.execute(
            sql`UPDATE image_annotations SET annotations = ${JSON.stringify(mergedAnnotations)}, "updatedAt" = NOW() WHERE id = ${existing.rows[0].id}`
          );
        } else {
          // Create new
          await db.execute(
  ...
  copyAnnotations: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      for (let i = 0; i < input.targetImageUrls.length; i++) {
        ...
        if (existing.rows?.length > 0) {
          await db.execute(
            sql`UPDATE image_annotations SET annotations = ${JSON.stringify(newAnnotations)}, "updatedAt" = NOW() WHERE id = ${existing.rows[0].id}`
          );
        } else {
          await db.execute(
...
Respond in JSON format with an array of findings.`
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (existing.rows[0].createdBy !== ctx.user.id && ctx.user.role !== 'admin') {
