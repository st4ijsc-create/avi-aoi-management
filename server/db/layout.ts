...
export async function getMachinePositionsByLayout(layoutId: number) {
  const db = await getDb();
  if (!db) return [];
  // Join with machines, stations, and lineStages to get stage info
  const result = await db
    ...
    .leftJoin(lineStages, eq(lineStages.stationId, stations.id))
    .where(eq(machinePositions.layoutId, layoutId));
  
  // Flatten the result to include machine and stage data directly in position
  return result.map(r => ({
    ...r.position,
    code: r.machine?.code,
    name: r.machine?.name,
    ...
    stageCode: r.stage?.code ?? null,
    stageOrderIndex: r.stage?.orderIndex ?? null,
