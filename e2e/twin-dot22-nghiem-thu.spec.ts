 ...
 *
 *     TWIN_E2E_ANH_DOT22=<thư mục của ĐỢT ĐANG CHẠY>  *     PLAYWRIGHT_BASE_URL=http://localhost:<cổng dist của đợt đang chạy>  *     npx playwright test e2e/twin-dot22-nghiem-thu.spec.ts --workers=1 --output <thư mục của đợt>/pw-output
 *
  `--config playwright.dot22.config.ts` (outputDir riêng `.qa-dot22/pw-output`, baseURL 127.0.0.1:3141).
 *
 ...
 * ⚠ Bỏ trống `TWIN_E2E_ANH_DOT22` thì `duongRaBangChung` tự đổi đường ra sang `.qa-dot22-lai-<mốc>`
