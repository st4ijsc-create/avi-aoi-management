#!/bin/sh
# PH-42 — server ĐO RIÊNG, cổng 3066.
#
# Vì sao không dùng `server.sh`/cổng 3064: agent PH-50b đang giữ 3064 (`dist-p50b-t2`)
# VÀ 3065 (`dist-p50b-t`) — kiểm bằng Win32_Process, xem báo cáo. `server.sh start` sẽ
# `taskkill` PID trong `server.pid` dùng chung, tức giết tiến trình của họ. Tệp này
# KHÔNG đụng `server.pid` và KHÔNG giết PID nào ngoài PID của chính nó.
cd /d/SOURCES/avi-aoi-management || exit 1
D=.qa-tapdoan
CONG=3066
PIDF=$D/_ph42-server.pid
case "$1" in
  start)
    ...
    PORT=$CONG NODE_ENV=production RATE_LIMIT_REDIS=false AUTH_RATE_LIMIT_PER_15MIN=5000 \
      nohup node "$T/index.js" > $D/_ph42-server-$2.out.log 2> $D/_ph42-server-$2.err.log &
