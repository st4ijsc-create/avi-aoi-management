...
try {
  const migrationPath = join(__dirname, "drizzle", "0095_p4c_genealogy_chain.sql");
  const migrationSQL = readFileSync(migrationPath, "utf-8");

  console.log("Running migration: 0095_p4c_genealogy_chain.sql");
  await sql.unsafe(migrationSQL);

  console.log("Migration 0095 completed successfully");
  console.log("Created: genealogy_chain (+ indexes)");
} catch (e) {
