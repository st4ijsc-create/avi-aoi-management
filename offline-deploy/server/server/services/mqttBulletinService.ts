...
export async function generateAndSendBulletin(setting: BulletinSetting): Promise<boolean> {
  ...
  try {
    ...
    // Use COALESCE to fall back to remark field for unresolved points (pointDefId=0)
    // Group by remark to prevent lumping all unresolved points into one "Unknown" entry
    const failPointsQuery = await db.select({
      pointId: schema.measurementResults.pointDefId,
      pointName: sql<string>`COALESCE(${schema.measurementPointDefs.name}, REPLACE(${schema.measurementResults.remark}, 'Point: ', ''))`,
      pointCode: sql<string>`COALESCE(${schema.measurementPointDefs.code}, REPLACE(${schema.measurementResults.remark}, 'Point: ', ''))`,
    ...
    .groupBy(
      ...
      schema.measurementResults.remark
    )
