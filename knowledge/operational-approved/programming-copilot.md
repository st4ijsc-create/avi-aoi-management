...
# Trợ lý Lập trình — xử lý sự cố & thao tác

## Thông tin đã xác minh từ mã nguồn
- **Đường dẫn**: `/programming-copilot`
- **Menu**: Kỹ thuật & Điều khiển (Nâng cao) › Trợ lý Lập trình
- **Quyền yêu cầu**: `machine_status`
- **Vai trò giới hạn**: không giới hạn theo vai trò
- **Module / license**: `null` — OPTIONAL (cần license)
- **Router tRPC**: không tìm được router khớp chính xác với route này. ⬜ **CHƯA GHI LẠI** — xác nhận màn hình này lấy dữ liệu từ đâu
...
- Thiếu quyền `machine_status` ⇒ màn hình có thể hiện rỗng thay vì báo lỗi rõ ràng.
- ⬜ **CHƯA GHI LẠI** — nguyên nhân nghiệp vụ đặc thù của màn hình này

## Các bước xử lý

1. Mở `/programming-copilot` (menu: Kỹ thuật & Điều khiển (Nâng cao) › Trợ lý Lập trình).
