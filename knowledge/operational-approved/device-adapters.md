...
# Adapter thiết bị — xử lý sự cố & thao tác

## Thông tin đã xác minh từ mã nguồn
- **Đường dẫn**: `/device-adapters`
- **Menu**: Thiết bị & Giám sát › Adapter thiết bị
- **Quyền yêu cầu**: `machine_control`
- **Vai trò giới hạn**: không giới hạn theo vai trò
- **Module / license**: `null` — OPTIONAL (cần license)
- **Router tRPC**: `deviceAdapterRouter` (server/routers/deviceAdapterRouter.ts, ~10 thủ tục)
- **Thao tác có thật ở backend**: `list`, `get`, `create`, `update`, `delete`, `testConnection`, `listByAdapter`
...
- Thiếu quyền `machine_control` ⇒ màn hình có thể hiện rỗng thay vì báo lỗi rõ ràng.
- ⬜ **CHƯA GHI LẠI** — nguyên nhân nghiệp vụ đặc thù của màn hình này

## Các bước xử lý

1. Mở `/device-adapters` (menu: Thiết bị & Giám sát › Adapter thiết bị).
