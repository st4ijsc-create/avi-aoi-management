#!/bin/sh
# ĐỢT 53 · D-1 — 48 ca Đợt 32 NGUYÊN TRẠNG (harness .qa-dot44/do32.mjs) trên 3054, hai vp, vai A/B/C + andon raised TẠM.
# TUẦN TỰ (G97). Đầu ra .qa-dot54/nen-*/ (G65/G130). Hàng tạm có trap (G111).
...
echo "--- andon raised TAM may 14 -> a2,a3,a4 @1600 ---"; node .qa-dot37/db.mjs andon-tam tao 14 | tee .qa-dot54/raised-tao.json | tr -d '\n'; echo
node .qa-dot44/do32.mjs --role=A --vp=1600x900 --cases=a2,a3,a4 --tag=$T/raised-1600x900 --base=$B > .qa-dot54/raised-node.log 2>&1; echo "node exit=$?"; grep -E "$L" .qa-dot54/raised-node.log
node .qa-dot37/db.mjs andon-tam xoa | tee .qa-dot54/raised-xoa.json | tr -d '\n'; echo
