...
# KHÔNG chạm dist/ (3000/3001/3008 phiên khác đang phục vụ nó).  sh .qa-dot54/build.sh <tag>
cd /d/SOURCES/_twin_wt || exit 1
T=/d/SOURCES/_twin_wt/.qa-dot54/dist-$1
...
(cd "$T" && find . -type f \( -path './public/*' -o -name 'index.js' -o -name 'index.cjs' \) | sort | xargs md5sum) > /d/SOURCES/_twin_wt/.qa-dot54/md5-dist-$1.txt
echo "md5 dong: $(wc -l < /d/SOURCES/_twin_wt/.qa-dot54/md5-dist-$1.txt) · bundle: $(grep -oE 'assets/index-[A-Za-z0-9_-]+\.js' $T/public/index.html)"
