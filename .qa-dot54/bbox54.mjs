...
const OUT = process.env.QA_OUT ?? `.qa-dot54/bbox-${TAG}`;
...
async function dangNhap(ctx) {
  const f = process.env.QA_STATE ?? `.qa-dot54/state-${TK.username}.json`;
...
writeFileSync(`${process.env.QA_OUT ?? ".qa-dot54"}/bbox-${TAG}.json`, JSON.stringify(kq, null, 2));
