...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        "flex h-8 w-full items-center justify-between gap-2 rounded-[var(--radius)] border border-border-strong bg-surface-muted px-2.5 py-1 text-sm outline-none transition-colors select-none focus-visible:border-[var(--focus)] focus-visible:ring-2 focus-visible:ring-[var(--focus)]/40 data-disabled:pointer-events-none data-disabled:opacity-50 data-popup-open:border-[var(--focus)]",
        className
      )}
      {...props}
    >
...
function SelectPopup({ className, children, ...props }: SelectPrimitive.Popup.Props) {
  return (
    <SelectPrimitive.Popup
      ...
      className={cn(
        "hmi-panel-glass hmi-scroll max-h-[var(--available-height)] min-w-[var(--anchor-width)] origin-(--transform-origin) overflow-y-auto rounded-[var(--radius-card)] border border-border py-1 text-popover-foreground shadow-[var(--elevation)] outline-none data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0",
        className
      )}
      {...props}
    >
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        ...
        "relative flex w-full cursor-default items-center gap-2 rounded-[var(--radius-sm)] py-1.5 pr-3 pl-7 text-sm outline-none select-none data-disabled:pointer-events-none data-disabled:opacity-50 data-highlighted:bg-navy-50 data-highlighted:text-primary-text dark:data-highlighted:bg-navy-800/60",
        className
      )}
      {...props}
    >
