 ...
 * claimed "the control rail never scrolls" and "E-STOP never moves between states" and both were
 * WRONG in ways only a real `scrollHeight`/`getBoundingClientRect()`/`elementFromPoint()` measurement
 * catches — a screenshot baseline alone does not: a masked, pixel-diffed screenshot of the E-STOP
 ...
 * (1280×800), across every machine class this component is shared by.
 */
test.describe("HMI safety rail — never scrolls, E-STOP never moves", () => {
  ...
  async function measureRail(page: Page) {
    return page.evaluate(() => {
      ...
      const estopBtn = Array.from(document.querySelectorAll("button")).find((b) => b.textContent?.includes("E-STOP"))
  ...
  for (const code of CLASSES) {
    test(`${code}: control rail never becomes scrollable, and E-STOP never moves, across stopped/running/paused/latched at 1280×800`, async ({
      page,
      request,
    }) => {
      ...
      // E-STOP latched — the worst case: banner + RESET both present, the tallest the rail ever gets.
      ...
      // 2) E-STOP's own on-screen box is BYTE-IDENTICAL across all four states — an operator's muscle
      ...
      for (const [i, r] of rest.entries()) {
        expect(r, `E-STOP rect in state #${i + 1} vs "stopped"`).toEqual(first)
      }
      ...
      expect(pageScrollable, "page/root scrolls while E-STOP is latched").toBe(false)
  ...
  for (const viewport of VIEWPORTS) {
    for (const theme of THEMES) {
      test(`${viewport.width}×${viewport.height} / ${theme}: E-STOP latched — rail fits with real margin, not flush against the panel edge`, async ({
        page,
        request,
      }) => {
