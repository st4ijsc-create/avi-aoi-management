...
/// <summary>
/// GĐ3 closeout WI-4 — <see cref="DeviceIdentityProvider"/>: <see cref="DeviceIdentityProvider.Current"/>
/// starts as whatever it was constructed with, <see cref="DeviceIdentityProvider.Rotate"/> actually swaps
/// it (a DIFFERENT fingerprint, reloadable from disk, same NodeId preserved), and concurrent rotations
/// don't corrupt the provider's own in-memory state (thread-safety — see that class's own doc comment).
/// </summary>
public sealed class DeviceIdentityProviderTests : IDisposable
{
    ...
    // Thread-safety smoke test (see DeviceIdentityProvider's own doc comment: the in-memory Current swap
    // is lock-protected — concurrent DISK writes racing each other is a separate, deliberately
    // out-of-scope concern this test does NOT exercise, since two Rotate() calls truly in flight at once
    // can legitimately have one of them lose a real file-system race over device-identity.bin, which is
    // not a bug in this class). What DOES have to hold: a reader spinning on Current while ONE Rotate() is
    // in flight on another thread must never observe a torn/partial identity (never throws, every read is
    // a complete, valid DeviceIdentity) — proving the lock actually protects the swap.
    [Fact]
