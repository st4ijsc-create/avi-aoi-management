...
| the **constructor** | **no, and this is the one that costs** | `MachineConfigStore`'s constructor only READS. `ProductConfigStore`'s and `SimulatedEcosystem`'s **seed and persist**, so a directory they cannot write to ends the process before the host exists — measured on the shipped exe, exit code `0xE0434352`. See `docs/startup-failure-posture.md` §3.5, rows 32/33. |
