...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        // 🔴 Review Minor 6. This catch is not decoration and it is not defensive style: on an UNHANDLED
        // exception the CLR terminates the process WITHOUT raising AppDomain.ProcessExit and without any
        // guarantee that a `finally` unwinds — so both nets that release ACL leases would miss the one case
        // they were written for. Catching here is what makes the `finally` below reachable on every path,
        // and there are real paths outside Attempt(): Fixture.Create and the store constructions.
        try
        {
            if (options.Acl)
            {
        ...
        {
            _problems++;
            Console.Error.WriteLine($"!! the probe threw and stopped measuring: {ex}");
        }
        finally
        {
    ...
    [
        ...
        // 🔴 R12 IS A DISCRIMINATOR, not another shape, and it exists because a review proposed a MECHANISM
        // for R1i and a proposed mechanism is a claim (§8.1(d)). R1i denies on the directory and — by
        // propagation — on the file. R1 denies only the directory; R5 denies only the file; both answer True.
        // If what makes R1i answer False were the INHERITANCE, this row answers True. If it is the deny
        // reaching BOTH objects, this row answers False, and the finding stops being about a flag.
        new("R12 dir AND file, no inherit", "BOTH objects, each explicitly: read + traverse on the DIRECTORY and all four read rights on the FILE, neither propagated",
            f => AclLease.All(
                AclLease.Deny(f.Gate, f.SettingsRoot, true, FileSystemRights.Read | FileSystemRights.Traverse),
                AclLease.Deny(f.Gate, f.SettingsFile, false, FileSystemRights.Read))),
    ...
    private static void PassTwo(SandboxGate gate)
    {
        ...
        Console.WriteLine("  Note that File.Exists SWALLOWS its own failures by contract, so unlike its two neighbours that");
        Console.WriteLine("  column can only ever print True or False; it can never report a denial as an exception name.");
        Console.WriteLine();
        Console.WriteLine("  THE OUTCOMES ABOVE ARE MEASURED. THE TWO EXPLANATIONS BELOW ARE INFERENCES — consistent with every");
        Console.WriteLine("  row, generated by none of them. Read the rows; treat the reasons as the weaker claim they are:");
        ...
        Console.WriteLine("  * File.Exists does NOT answer off the file alone, and it does not answer off the directory alone.");
        Console.WriteLine("    Read the column: every row that withholds the rights on only ONE of the two objects reports");
        Console.WriteLine("    True — including R5, which denies ReadAttributes directly on the file. The rows that report");
        Console.WriteLine("    False are the ones where the deny reached BOTH the directory and the file (R1i by propagation,");
        Console.WriteLine("    R12 by two explicit rules), plus C1 where there is genuinely no file. THAT is the measured rule.");
        Console.WriteLine("    🔴 An explanation for it — the attribute lookup being answered from the parent's directory entry");
        Console.WriteLine("    while the parent permits it, and falling back to a parent enumeration that the parent also has to");
        Console.WriteLine("    permit — is an INFERENCE consistent with every row, not a measurement. R12 is what rules out the");
        Console.WriteLine("    rival explanation that the INHERITED_ACE flag itself matters; nothing here rules out a third.");
    }
