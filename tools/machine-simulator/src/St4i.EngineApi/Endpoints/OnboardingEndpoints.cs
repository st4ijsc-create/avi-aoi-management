...
/// symmetrically for BOTH Demo and Live, since either mode can produce a freshly-provisioned machine.</summary>
public static class OnboardingEndpoints
{
    public static void MapOnboardingEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        {
            var result = await svc.ClaimAsync(request, ct).ConfigureAwait(false);
        ...
        {
            var result = await svc.EnrollAsync(request, ct).ConfigureAwait(false);
            result = OnboardingFleetJoin.JoinFleetIfProvisioned(fleetHost, result, request.SerialNumber, request.MachineType);
            return Results.Ok(result);
        });
