...
//
// 🔴 THE LOOP ITSELF LIVES IN TagMapStartupIngestion, NOT HERE, AND THAT IS THE FIX FOR A MEASURED
// HOLE. It was inline, and Task 4's own sweep found that removing its swallow reddened nothing (row P2)
// and typo-ing the folder name reddened nothing (row P3) — no test could reach a loop that resolved its
// own directory from AppContext.BaseDirectory. Same hole, same cause and same fix as D-1's extraction of
// the connectors.json dispatch into ConnectorsJsonRegistration; see that class' doc comment and this one's.
// What remains here is the composition: resolve the services, read the bindings, hand over a directory.
...
St4i.EngineApi.HmiModel.TagMapStartupIngestion.IngestAll(
    St4i.EngineApi.HmiModel.TagMapStartupIngestion.ResolveDirectory(),
    tagMapRegistry.SnapshotBindings()
        .Select(b => (b.InstanceId, b.MachineCode, tagMapRegistry.KindOf(b.InstanceId)))
        .ToList(),
    tagIngestion,
    app.Services.GetRequiredService<ILoggerFactory>().CreateLogger("HmiTagIngestion"));
// ───────────────────────────────────────────────────────────────────────────────────────────────────
