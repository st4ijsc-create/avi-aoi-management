...
| Chưa xuất bản gì thì sao? | Vẫn vẽ đúng tài liệu đang ship của lớp máy ấy | §5-bis. `GET /v1/screens/{id}` trả 404 cho mã chưa ai khai (chủ ý — xem `HmiScreenEndpoints`), và 404 rơi thẳng về nhánh dự phòng. Ba màn baseline không đổi MỘT ĐIỂM ẢNH. 🔴 **SỬA sau review toàn nhánh (M-2):** bằng chứng cũ ở đây là một phép `grep` ("không chỗ nào trong kho này ghi `machine-scrw-01`/`machine-aoi-01`/`machine-iot-01`") và phép `grep` ấy ĐÃ HẾT rỗng **ngay trong chính nhánh này** — vòng security (`6dad4d2c`) thêm `HmiScreenEndpointsTests.cs:579` `PUT` `machine-aoi-01` có chủ ý, vào một kho tạm của `WebApplicationFactory`, nên KẾT LUẬN không đổi và không baseline nào bị đe doạ. Cái đổi là BẰNG CHỨNG. Vật chứng đứng vững là bài đo SỐNG: `43-editor-acceptance.spec.ts` khẳng định điều kiện 404 cho cả ba máy baseline trên kho THẬT, và gọi tên máy khi đỏ. Vùng tên `machine-` **không được cửa ghi bảo vệ** — xem mục "cái giá" bên dưới |
