...
- **✅ P3 batch-2 XONG** (commit `47e56e1c`, mig 0284, tsc 0 + 128 test): StreamProcessor wiring (QĐ#5, boot + honest NATS-readiness; kích hoạt thật cần `npm i nats` + NATS_URL) · orphan-reaper DB-diff (dry-run, chạy thật phát hiện 6 orphan) · version-rollback API · station_traces scope (CASE #8) · syncMeasurementPoints bump→atomic (CASE #12) · multi-factory doc (QĐ#4).
- **⏳ Còn lại (đợt chuyên biệt — cần thiết kế schema riêng):** **product-variant** (schema variant + sync contract, tránh bùng nổ productModel) · **fiducial registration** server-side affine (≥2 fiducial → transform toạ độ) · **image single-tx thật** (gộp header+measurement 1 physical tx — đổi flow, phá durability-test model; P2 compensation đã giảm phần lớn residual). + kích hoạt NATS thật (ops: cần server NATS).
