...
| **(3) Phân bố lệch mới** | ✅ Đo được, **101 mẫu** trên nền hợp lệ: p50 **15**, p90 **210**, p95 **210**, max **1.002** MiB (max ở **trạng thái ổn định** chỉ **210**), **báo động 0/101 = 0 %**. Pha 1: p50 536, p90 664, p95/max 16.335, **100 % báo động kể từ lượt `commit` đầu**. **Sàn cấu trúc đã sập từ 536-882 MiB xuống ≤ 210 MiB.** |
...
**Nhánh RESAMPLE của Task 1 có thể NUỐT TRỌN model đang nạp vào NỀN — vĩnh viễn.**
Tái hiện **2/2 lượt**. Hệ quả đo được: một lệch **41,6 MiB** biến thành lệch **−16.700 MiB
đứng mãi**, báo động **100 % mọi nhịp** cho tới khi **khởi động lại tiến trình**.
Đây là lỗi **tệ hơn** cả trạng thái trước Pha 1.5. Chi tiết + bằng chứng pháp y ở **§5**.
⇒ **Mọi khuyến nghị ngưỡng ở §6 chỉ có nghĩa SAU khi khoản này được xử lý** — không ngưỡng nào
cứu được một cái nền sai 17 GiB.
...
Điều kiện: nền được chụp bằng **cùng một thước** với mọi phép so sau đó, và **không** rơi vào cửa
sổ chưa-commit. Đây là con số trả lời câu hỏi *"ba task sửa lỗi đo để lại bao nhiêu lệch tồn dư?"*.
...
| **A3 — 30B + buffer suy luận** | 20 | 134 | **140** | 140 | 140 | **140** | 0/20 |
| **A4 — 30B + embedder + embed-ctx** | 25 | 210 | **210** | 386 | 386 | **386** | 0/25 |
| **A5 — cửa sổ nạp (chưa commit)** | 3 | 134 | 136 | 1002 | 1002 | **1.002** | 0/3 |
...
| **p90** | **664** | **210** |
| **p95** | **16.335** | **210** |
| **max** | **16.335** | **1.002** (max ở trạng thái **ổn định**: **210**) |
| tỉ lệ báo động | **20/35 = 57 %**, và **18/18 = 100 %** kể từ lượt `commit` đầu | **0/101 = 0 %** |
...
| Task 3 — băng dung sai chỉ phía âm | tới −16.335 MiB | ✅ Cửa sổ nạp 30B: `drift = −17.290 MiB` với `pendingBytes = 16.871` ⇒ ngưỡng âm = −(512+16.871) = **−17.383** ⇒ **`alarm = 0`**. Pha 1 cùng ca này **báo động**. Xác nhận ở **21 mẫu** liên tiếp qua 2 lượt. |
...
## 5. ★★ PHÁT HIỆN CHẶN — nhánh RESAMPLE nuốt model đang nạp vào NỀN, VĨNH VIỄN
...
### 5.2 Vì sao tiền đề của `captureVramBaseline()` SAI
...
kịp**, **không** nói thiết bị còn trống. Tiền đề đúng cho **lượt chụp đầu lúc boot** (khi thật sự
chưa nạp gì) nhưng **sai cho nhánh RESAMPLE** — nhánh này Task 1 mới thêm và **có thể nổ ở BẤT KỲ
thời điểm nào**, kể cả giữa lượt nạp. Lá chắn `if (raw < committedBytes)` (`:199`) **không bắt
...
### 5.5 Xác suất trong sản xuất — phụ thuộc NHỊP, và đó mới là điểm chết người

Cửa sổ rủi ro = `[handle gắn → commit]`. Đo hôm nay cho 30B (cache file nóng): **~10,5 s**;
Pha 1 §3.5 đo **11 – 43 s** (cache lạnh). Nhánh resample chỉ nổ **một lần duy nhất mỗi tiến
trình** — ở nhịp đối chiếu **đầu tiên sau khi handle gắn**.

| Kịch bản | Xác suất nhiễm nền |
|---|---|
| **All-in-one boot chuẩn, nhịp 60 s** — `startVramReconciler()` ở `backgroundJobs.ts:141` chạy **ngay trước** `initDeepModelWarmup()` (`:155`, `setTimeout 3000`) ⇒ handle gắn ~T0+3 s, commit ~T0+14…46 s, nhịp đầu ở **T0+60 s** | **thấp** — nhịp đầu rơi **sau** commit. **Đây là lý do Pha 1 quan sát được +738,6 chứ không phải −16,7 GiB.** |
...
```
⚠ **`vram_events` id 69 – 121 (2026-08-03 03:52 – 04:01) là do TIẾN TRÌNH CHẨN ĐOÁN của Task 5
ghi, KHÔNG phải sản xuất** (37 dòng `drift` trong đó đến từ lượt tái hiện §5). **Phải lọc bằng
`id > <ID0>` hoặc `"createdAt" >= <T0>`.**
...
2. **(c)** **không** dòng `baseline` nào có `base_mib` > **2.048** (nền hợp lệ ~1.000 MiB).
   Một dòng vượt ngưỡng đó = **§5 đã nổ ⇒ FAIL, và mọi số ở (b) sau thời điểm đó là RÁC**.
3. **(b)** số dòng `drift` **nhỏ**, và mỗi dòng giải thích được bằng `detail.leases`.
4. `nvidia-smi` cuối kỳ trở về nền (~1.000 MiB) sau khi tắt app; không giấy phép treo.
...
| # | Mục | Mức |
|---|---|---|
| **T5-1** | **§5 — nhánh RESAMPLE nuốt model đang nạp vào nền, không tự lành.** Hướng khả dĩ (chưa thẩm định): hoãn resample khi `pendingBytes > 0`; **hoặc** dùng `raw − Σ leaseBytes()` cho **riêng** nhánh resample; **hoặc** không cho `baselineCaptured` đóng băng khi lượt chụp rơi vào cửa sổ pending. **Mỗi phương án phải trả lời trước: "nếu chính nhánh này kích hoạt SAI thì bao lâu nó tự lành?"** | **CHẶN** |
...
| Bộ | Thứ tự | Model | n | File thô |
|---|---|---|---|---|
| B1 | prod (nền `smi` trước, rồi nạp) | Qwen3-30B-A3B UD-Q4_K_XL | 76 | `t5-drift-30b.csv` |
| B2 | prod, **có ghi DB** | idem | 57 | `t5-drift-prod2.csv` |
| A | clean (nạp trước, chụp nền sau khi mọi lease đã commit) | idem + Qwen3-Embedding-0.6B | 68 | `t5-drift-clean.csv` |
...
```
NGHỈ, sổ rỗng, thước smi        :  0×9, -4×6, -8×6, -15×5, +3, +14           (n=33)
30B thường trú, trước suy luận  :  0 ×20                                     (n=20)
lượt suy luận ĐẦU (buffer lười) : +134×6, +136                               (n= 7)
30B + buffer, ổn định           : +140×15                                    (n=15)
nạp embedder (chưa commit)      : -1002, -386×4                              (n= 5)
30B + embedder + embed-ctx      : +210×21                                    (n=21)
                                   ────────────────────────────────────────────────
...
- Ghi vào DB: **52 dòng** `vram_events` (id 69 – 121) do tiến trình chẩn đoán sinh ra — **đã đánh
  dấu ở §7.1** để lượt 24 giờ lọc ra.
