...
- [x] Dạy giới hạn qua UI thật (hoặc caller tRPC) → đẩy lại mẫu kết quả thật ⇒ `specGate.dat + truot > 0` lần đầu trên đường v2 (trước đó `khongGioiHan` 100%) — dán số trước/sau kèm `current_database()`. — `343f8af9` `congRaKhoiC.db.test.ts`, `aoi_management_test`: TRƯỚC dat 0/truot 0/chuaDay 32/khongGioiHan 16 → SAU dạy 3: dat **2**/truot **1**/chuaDay 32/khongGioiHan 13; `mayTuMauThuan` {48,0} không đổi; versions 0→3; đột biến bỏ dạy ⇒ `expected 0 to be greater than 0`. BG-112: 32/48 lệch fixture. ⚠ **Lượt 9 (C-1/BG-118): phép đo này chạy với `SPEC_GATE_SNAPSHOT_ENABLED` TẮT** — chứng minh "cổng chấm" nhưng KHÔNG chứng minh "chấm theo giới hạn lúc đo"; ca nguy hiểm (bo CŨ WAL + siết SAU) ngoài phạm vi. Vòng sửa 9 SERVER tham số hoá mệnh đề 3 CẢ HAI trạng thái cờ. Tick này **có điều kiện** tới khi vòng sửa hạ cánh.
