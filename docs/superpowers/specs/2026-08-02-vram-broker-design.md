...
| 🔴 Suy biến **KHÔNG** an toàn — đính chính 2026-08-04 | Bản đầu viết *"ba trạng thái mù ⇒ rơi về chỉ-sổ, suy biến an toàn"*. **SAI.** Vì `max(L, A) ≥ L`, **`attributable = null` là CHẶN TRÊN của mọi headroom** ⇒ chỉ-sổ là nhánh **RỘNG RÃI NHẤT**, mà sổ chỉ nối **14/159** dòng. **Mọi đường sinh `blind` là một đường VÔ HIỆU HOÁ lớp bảo vệ** — đã đếm **11 đường**; tệ nhất không phải đầu dò hỏng mà là lease `local-trainer` **ttl 2 giờ, đang bật, cố ý không commit** ⇒ mù **hàng giờ**. ⇒ `blind`/`unverified` phải làm hệ **CHẶT HƠN**; một nền **nhiễm** vẫn chặt hơn **chỉ-sổ**, nên **chụp-và-đánh-dấu** đúng hơn **từ chối**. |
