...
export default function TraceabilityLineage() {
  ...
  // doc 63 DEP-05 (AUD-04 / IPC-1782a) — FULL cross-station history: chain ∪ inspections
  // ∪ process_results ∪ installations qua genealogy.getFullHistory (một timeline, hết
  // phân mảnh "tab serial bỏ bước trạm"). Honest: found=false → không render gì thêm.
  const fullHistory = trpc.genealogy.getFullHistory.useQuery(
    { serialNumber: serialQuery },
    { enabled: mode === "serial" && serialQuery.length > 0 },
  );
  const fh = fullHistory.data?.found ? fullHistory.data.record : null;
