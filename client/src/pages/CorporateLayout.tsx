...
export default function CorporateLayout() {
  ...
  const { data: capacityByFactory } = trpc.corporateFactoryStats.capacityByFactory.useQuery();
  const { data: alertSummary } = trpc.corporateFactoryStats.alertSummaryByFactory.useQuery();
  ...
  const factoriesWithStats = useMemo(() => {
    ...
    return factories.map((factory) => {
      ...
      const cap = capacityByFactory?.find(c => c.factoryCode === factory.code);
      const stats = {
  ...
  }, [factories, workshops, yieldByFactory, capacityByFactory]);
  ...
  useEffect(() => {
    ...
    factoriesWithStats.forEach((factory) => {
      ...
      const capBarY = y + boxHeight - 65;
      const capBarWidth = boxWidth - 30;
      const machineTotal = (factory as any).machineCount || 0;
      const machineActive = (factory as any).activeMachineCount || 0;
      const capRatio = machineTotal > 0 ? machineActive / machineTotal : 0;
      const capColor = capRatio >= 0.8 ? "#10b981" : capRatio >= 0.5 ? "#f59e0b" : "#ef4444";

      ctx.fillStyle = "#94a3b8";
      ctx.font = "10px sans-serif";
      ctx.fillText("Capacity", x + 20, capBarY - 3);
      ctx.fillStyle = "#cbd5e1";
      ctx.font = "bold 11px sans-serif";
      ctx.fillText(`${machineActive}/${machineTotal}`, x + 75, capBarY - 3);

      ctx.fillStyle = "rgba(100, 116, 139, 0.2)";
      ctx.beginPath();
      ctx.roundRect(x + 15, capBarY + 2, capBarWidth, 6, 3);
      ctx.fill();
      ctx.fillStyle = capColor;
      ctx.beginPath();
      ctx.roundRect(x + 15, capBarY + 2, capBarWidth * capRatio, 6, 3);
      ctx.fill();
      ...
      // Alert indicator (red dot at top-right if factory has unacknowledged alerts)
      const factoryAlerts = alertSummary?.[factory.id];
      if (factoryAlerts && factoryAlerts.length > 0) {
        ctx.fillStyle = "#ef4444";
        ctx.beginPath();
        ctx.arc(x + boxWidth - 12, y + 12, 8, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#ffffff";
        ctx.font = "bold 10px sans-serif";
        ctx.textAlign = "center";
        ctx.fillText(String(factoryAlerts.length), x + boxWidth - 12, y + 16);
        ctx.textAlign = "start";
      }
    });
  ...
  }, [viewMode, factoriesWithStats, selectedFactory, zoom, workshops?.length, factoryPositions, panOffset, t, isFullscreen, draggedFactoryId, alertSummary]);
