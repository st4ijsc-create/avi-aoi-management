// DOT 59 — tom tat 24 trang thai thi giac theo CAC O BAT BIEN (doc tu tong.json).
import { readFileSync, writeFileSync, renameSync } from "node:fs";
const LANG = process.argv[2] ?? "vi";
const d = JSON.parse(readFileSync(`.qa-dot59/d3-${LANG}/tong.json`, "utf8"));
...
const ra = `=== DOT 59 · thi giac lang=${LANG} tren dist MOI (cong 3059) ===
so trang thai do : ${n}
...
`;
writeFileSync(`.qa-dot59/HQ-thigiac-${LANG}-tomtat.txt.tmp`, ra);
renameSync(`.qa-dot59/HQ-thigiac-${LANG}-tomtat.txt.tmp`, `.qa-dot59/HQ-thigiac-${LANG}-tomtat.txt`);
console.log(ra);
