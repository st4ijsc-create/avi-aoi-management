// ★★★ ĐỢT 59 (mục A, bước 3) — THÊM TRẠNG THÁI **`twin/thu-mo-panel`**: 12 → 13 trạng thái × 2 vp
//   = **24 → 26 ca** mỗi ngôn ngữ. Lỗ đo mà QA lần 10 chỉ ra: cả 12 trạng thái cũ đều đo panel
//   trái ở trạng thái MỞ (hoặc mở sẵn từ URL); không ca nào đi qua *"thu rồi mở lại"* — nên
//   nhãn 3D nằm dưới tay nắm ≥ 17 s sống sót qua 24 ca × 2 ngôn ngữ mà không ai thấy.
//   Trạng thái mới bấm `nut-thu-trai` hai lần rồi chờ 3 s trước khi đo — KHÔNG đổi cỡ cửa sổ
//   (đổi cỡ là thứ DỌN được bệnh; đo sau khi dọn là tự thoả).
...
//   node .qa-dot59/thigiac59.mjs --lang=vi [--base=http://localhost:3059] [--vp=1600x900,1280x720]
...
const OUT = process.env.QA_OUT ?? `.qa-dot59/d3-${LANG}`;
...
async function dangNhap(ctx) {
  const f = process.env.QA_STATE ?? `.qa-dot59/state-${TK.username}.json`;
...
try {
  for (const vp of VPS) {
    ...
    /* ══ ★★★ ĐỢT 59 (mục A) — twin/thu-mo-panel: THU rồi MỞ LẠI panel trái ══
       `LopNhan` đọc vùng cấm trong `useFrame`, mà `frameloop="demand"`. Tay nắm đổi `left` qua
       `transition-[left] 200ms` — `ResizeObserver` MÙ với dời chỗ ⇒ khung cuối được vẽ lúc tay nắm
       còn đang trượt ⇒ nhãn ở lì dưới nút. Trạng thái này là ca duy nhất trong lưới đi qua đó. */
    /* ⚠ PHẢI mở ở `?thu=nhanTatCa`. ĐO ĐƯỢC (Đợt 59, ablation lần 1): ở chính sách nhãn MẶC ĐỊNH
       (`chỉ tên máy bất thường`) màn chỉ còn 3–4 nhãn và KHÔNG cái nào rơi vào dải tay nắm ⇒ ca này
       XANH cả trên bản dựng CHƯA VÁ. Một ca không biết KÊU trên ca dương đã biết thì không phải lưới.
       Với `?thu=nhanTatCa` (45 nhãn) bản chưa vá cho 2 nhãn / 210 px² dưới tay nắm. */
    await page.goto(`${BASE}/twin?thu=nhanTatCa`, { waitUntil: "domcontentloaded" });
    await page.waitForSelector('[data-testid="man-twin-van-hanh"] canvas', { timeout: 90_000 }).catch(() => {});
    await page.waitForTimeout(7000);
    const thuMo = { coNut: (await page.locator('[data-testid="nut-thu-trai"]').count()) > 0, rongTruoc: null, rongThu: null, rongSau: null };
    const rongPanel = () => page.evaluate(() => { const e = document.querySelector('[data-testid="panel-trai"]'); return e ? Math.round(e.getBoundingClientRect().width) : null; });
    thuMo.rongTruoc = await rongPanel();
    if (thuMo.coNut) {
      await page.getByTestId("nut-thu-trai").click(); await page.waitForTimeout(2500); thuMo.rongThu = await rongPanel();
      await page.getByTestId("nut-thu-trai").click(); await page.waitForTimeout(3000); thuMo.rongSau = await rongPanel();
    }
    // Bất biến của CHÍNH trạng thái này: panel phải thật sự thu (>0 → 0) rồi mở lại (0 → >0);
    // nếu không, ca "thu rồi mở" chưa hề xảy ra và mọi số 0 phía dưới là 0 GIẢ (G146).
    thuMo.thatSuThuMo = thuMo.rongTruoc > 0 && thuMo.rongThu === 0 && thuMo.rongSau > 0;
    // Bất biến thứ hai: phải CÓ nhãn để mà che. 0 nhãn ⇒ "0 bị che" là 0 GIẢ (G146).
    thuMo.soNhanSauMo = await page.locator('[data-testid="nhan-may-twin3d"]').count();
    await doTrangThai("twin", "thu-mo-panel", "man-twin-van-hanh", { thuMo });
    console.log(`   [${vp}] twin/thu-mo-panel: panel ${thuMo.rongTruoc}→${thuMo.rongThu}→${thuMo.rongSau} px · thật sự thu/mở ${thuMo.thatSuThuMo} · nhãn sau khi mở ${thuMo.soNhanSauMo}`);
