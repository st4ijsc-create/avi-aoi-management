// ĐỢT 59 — BẢN SAO tomtat58 (44 luật add() Y NGUYÊN), chỉ đổi thư mục cột cuối sang .qa-dot59/d1head.
// ĐỢT 58 — BẢN SAO tomtat-D1 Đợt 44, GIỮ NGUYÊN 48 LUẬT; chỉ đổi NGUỒN hai cột cuối:
//   cột 4 = lượt chạy Đợt 57 (.qa-dot57/nen-*), cột 5 = lượt chạy Đợt 58 ($QA59_DIR).
//   QA59_DIR=.qa-dot58/d1head node .qa-dot58/tomtat58.mjs
// ĐỢT 44 · D-1 — BẢNG KẾT CỤC NĂM CỘT: Đợt 32 (.qa-dot32/nen*) → Đợt 37 (.qa-dot37/nen-*) → Đợt 39 (.qa-dot39/nen-*) → Đợt 41 (.qa-dot41/nen-*) → HEAD Đợt 44 (.qa-dot44/nen-*, mạng CDN chặn), sinh TỪ TỆP THÔ. 48 luật phán quyết GIỮ NGUYÊN của tomtat-D1 Đợt 37 (tệp này sinh bằng .qa-dot41/tao-tomtat.cjs).
...
const D41 = { A: ".qa-dot57/nen-1600x900", A12: ".qa-dot57/nen-1280x720", R: ".qa-dot57/raised-1600x900" }; /* ĐỢT 58: cột 4 = LƯỢT CHẠY CỦA ĐỢT 57 (để đối chiếu trực tiếp) */
const P58 = process.env.QA59_DIR || ".qa-dot59/d1head";
const MOI = { A: `${P58}-1600x900`, A12: `${P58}-1280x720`, R: `${P58}-raised-1600x900` };
const rows = [];
// luat: (json) => [giaTriHienThi, phanQuyet]  — phanQuyet ∈ ĐẠT | SAI | HỎNG | CHẶN-ĐÚNG | N/A
const add = (id, moTa, dir, file, luat, ghi) => {
  const c = j(`${CU[dir]}/${file}`), d = j(`${D37[dir]}/${file}`), e = j(`${D39[dir]}/${file}`), f = j(`${D41[dir]}/${file}`), m = j(`${MOI[dir]}/${file}`);
  ...
  rows.push({ id, moTa, file: `${file}`, cu: gc, pcu: pc, d37: gd, pd37: pd, d39: ge, pd39: pe, d41: gf, pd41: pf, moi: gm, pmoi: pm, ghi: ghi ?? "" });
};
...
if (process.argv.includes("--md=1")) {
  console.log("| # | Ca | Đợt 32 | | Đợt 37 | | Đợt 39 | | Đợt 41 | | HEAD Đợt 44 | | Ghi chú |\n|---|---|---|---|---|---|---|---|---|---|---|---|---|");
  for (const r of rows) console.log(`| ${r.id} | ${r.moTa} \`${r.file}\` | ${r.cu} | **${r.pcu}** | ${r.d37} | **${r.pd37}** | ${r.d39} | **${r.pd39}** | ${r.d41} | **${r.pd41}** | ${r.moi} | **${r.pmoi}** | ${r.ghi} |`);
} else for (const r of rows) console.log(`${String(r.id).padStart(2)} ${r.pcu.padEnd(10)} → ${r.pd37.padEnd(10)} → ${r.pd39.padEnd(10)} → ${r.pd41.padEnd(10)} → ${r.pmoi.padEnd(10)} ${r.moTa} | 32: ${r.cu} | 37: ${r.d37} | 39: ${r.d39} | 41: ${r.d41} | 44: ${r.moi}${r.ghi ? " | " + r.ghi : ""}`);
console.log(`\nN=${rows.length} · ĐỢT 32 ${JSON.stringify(dem("pcu"))} · ĐỢT 37 ${JSON.stringify(dem("pd37"))} · ĐỢT 39 ${JSON.stringify(dem("pd39"))} · ĐỢT 41 ${JSON.stringify(dem("pd41"))} · HEAD 44 ${JSON.stringify(dem("pmoi"))}`);
const doi = rows.filter((r) => r.pd41 !== r.pmoi); console.log(`Đổi phán quyết Đợt 41 → HEAD Đợt 44: ${doi.length}: ${doi.map((r) => `#${r.id} ${r.pd41}→${r.pmoi}`).join(", ") || "(không)"}`);
