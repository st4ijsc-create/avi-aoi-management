...
//   node .qa-dot59/bbox45.mjs --tag=truoc|sau|go-<muc> [--base=http://localhost:3059] [--lang=vi]
// Ghi .qa-dot59/bbox-<tag>.json + in "[vp] M<n> ĐẠT|TRƯỢT chi tiết". Mỗi mục có tiêu chí NÓI RA ở đầu hàm.
...
const OUT = process.env.QA_OUT ?? `.qa-dot59/bbox-${TAG}`;
...
writeFileSync(`${process.env.QA_OUT ?? ".qa-dot59"}/bbox-${TAG}.json`, JSON.stringify(kq, null, 2));
