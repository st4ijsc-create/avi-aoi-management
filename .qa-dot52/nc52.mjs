// ĐỢT 49 · K7-NC49 (chép từ .qa-dot48/k7-nhan-che.mjs — KHÔNG sửa bản của QA)
//   THÊM: cặp (nhãn máy A ∩ HÌNH CHIẾU KHỐI máy B, A≠B) + diện tích giao, đọc hộp THẬT của CÙNG một khung
//   qua __demTuongTac.hopNhanDaVe()/hopKhoiMay(); và BẤM THẬT tối đa 3 máy bị phủ (không phải 1).
...
//   Rồi BẤM THẬT vào một máy bị phủ (nếu có) ⇒ URL đi tới máy nào (nhãn hay khối?). Ghi .qa-dot49/k7/nc-*.json + PNG.
//   node .qa-dot49/k7-nhan-che.mjs [--base=http://localhost:3049] [--vp=1600x900,1280x720] [--fc=1]
import { chromium } from "@playwright/test";
import { existsSync, mkdirSync, readFileSync, writeFileSync } from "node:fs";
const arg = (k, d) => { const m = process.argv.find((a) => a.startsWith(`--${k}=`)); return m ? m.slice(k.length + 3) : d; };
const BASE = arg("base", "http://localhost:3049"); const VPS = arg("vp", "1600x900,1280x720").split(","); const FC = arg("fc", "1") === "1";
const OUT = process.env.NC_OUT || ".qa-dot49/k7-va"; mkdirSync(OUT, { recursive: true });
...
const CENSUS = (manTid) => {
  ...
  // ★ Đợt 49 — hộp THẬT của cùng một khung (không suy từ DOM): nhãn đang vẽ + hình chiếu khối từng máy.
  const hopNhan = window.__demTuongTac?.hopNhanDaVe?.() ?? [];
  const hopKhoi = window.__demTuongTac?.hopKhoiMay?.() ?? [];
  ...
  let soCapNhanKhoi = 0, dienTichNhanKhoi = 0; const capChiTiet = [];
  for (const n of hopNhan) for (const k of hopKhoi) {
    if (k.machineId === n.machineId) continue;
    const d = giao(n.hop, k.hop);
    if (d > 0) { soCapNhanKhoi += 1; dienTichNhanKhoi += d; capChiTiet.push({ nhanCua: n.machineId, deKhoi: k.machineId, px2: Math.round(d) }); }
  }
  const dem = window.__demNhan ?? null;
  return { url: location.href, soNhanDaVe: hopNhan.length, soKhoiChieu: hopKhoi.length, soCapNhanKhoi, dienTichNhanKhoi: Math.round(dienTichNhanKhoi), capChiTiet: capChiTiet.slice(0, 40), demNhan: dem ? { ve: dem.ve, tong: dem.tong, biGiau: dem.biGiau, biChe: dem.biChe, chongLap: dem.chongLap, vuotMep: dem.vuotMep, deKhoiKhac: dem.deKhoiKhac, deKhoiKhacDoLai: dem.deKhoiKhacDoLai, soHopKhoi: dem.soHopKhoi } : null, demBadge: window.__demBadge ? { ve: window.__demBadge.ve, tong: window.__demBadge.tong, soAn: window.__demBadge.soAn, biChe: window.__demBadge.biChe, doiCho: window.__demBadge.doiCho } : null, chipCanhBaoAn: (() => { const el = document.querySelector('[data-testid="chip-canh-bao-bi-an"]'); if (!el) return null; const r = el.getBoundingClientRect(); return { chu: el.textContent?.trim(), so: el.getAttribute("data-so"), rect: { x: Math.round(r.left), y: Math.round(r.top), w: Math.round(r.width), h: Math.round(r.height) }, trongCanvas: r.left >= cv.left - 1 && r.top >= cv.top - 1 && r.right <= cv.right + 1 && r.bottom <= cv.bottom + 1 }; })(), soLanDungLo: window.__demTuongTac?.soLanDungLo ?? null, cv: { x: cv.left, y: cv.top, w: cv.width, h: cv.height }, soTrongKhung: ds.length, soBamDuoc: bamDuoc.length, soNhan: nhan.length, soBadge: badge.length, peNhan: [...new Set(nhan.map((n) => n.pe))], peBadge: [...new Set(badge.map((b) => b.pe))], nhan: nhan.map((n) => ({ id: n.id, chu: n.chu, rect: { x: Math.round(n.rect.x), y: Math.round(n.rect.y), w: Math.round(n.rect.w), h: Math.round(n.rect.h) } })), may, biNhanKhacPhu: bamDuoc.filter((m) => m.nhanPhuKhac.length).map((m) => ({ id: m.id, X: m.X, Y: m.Y, nhan: m.nhanPhuKhac })), biBadgeKhacPhu: bamDuoc.filter((m) => m.badgePhuKhac.length).map((m) => ({ id: m.id, X: m.X, Y: m.Y, badge: m.badgePhuKhac })), biNhanMinhPhu: bamDuoc.filter((m) => m.nhanPhu.includes(m.id)).length };
};
  ...
  for (const vp of VPS) {
    ...
    for (const [duong, man] of MAN) {
      ...
      try {
        ...
        const in1 = (c, ten) => `${ten}: trong khung ${c.soTrongKhung} · bấm được ${c.soBamDuoc} · CẶP nhãn∩khối-máy-khác ${c.soCapNhanKhoi} (${c.dienTichNhanKhoi} px², nhãn vẽ ${c.soNhanDaVe}, khối chiếu ${c.soKhoiChieu}, deKhoiKhac ${c.demNhan?.deKhoiKhac}/${c.demNhan?.deKhoiKhacDoLai}) · badge ve/tong/soAn/biChe ${c.demBadge?.ve}/${c.demBadge?.tong}/${c.demBadge?.soAn}/${c.demBadge?.biChe} · chip cảnh báo ẩn ${c.chipCanhBaoAn ? `"${c.chipCanhBaoAn.chu}" trongCanvas=${c.chipCanhBaoAn.trongCanvas}` : "KHÔNG CÓ"} · nhãn ${c.soNhan} (pe ${c.peNhan}) badge ${c.soBadge} (pe ${c.peBadge}) · tâm bị NHÃN máy KHÁC phủ ${c.biNhanKhacPhu.length} [${c.biNhanKhacPhu.map((x) => `${x.id}(${tenMay[x.id] ?? "?"})@${x.X},${x.Y}←${x.nhan.join("|")}`).join(" ; ").slice(0, 260)}] · bị BADGE máy khác phủ ${c.biBadgeKhacPhu.length} [${c.biBadgeKhacPhu.map((x) => `${x.id}←${x.badge.join("|")}`).join(" ; ").slice(0, 160)}] · tâm trong nhãn CỦA MÌNH ${c.biNhanMinhPhu}`;
        ...
        // ★ Đợt 49 — bấm MỌI máy bị phủ (tối đa 3), không chỉ máy đầu tiên.
        kq.bamNhieu = [];
        for (const bam2 of kq.sauKeo.biNhanKhacPhu.slice(0, 3)) {
          // ★★★ ĐỢT 49 — ĐO LẠI TOẠ ĐỘ TRƯỚC MỖI CÚ BẤM. Bản đầu dùng toạ độ của khung "sau kéo"
          //   cho cả 3 cú bấm, nhưng giữa hai cú có `goBack` ⇒ camera về tư thế khác ⇒ cú thứ 2/3
          //   bấm vào TOẠ ĐỘ CŨ trên một cảnh MỚI. Đó là lý do cú thứ 3 hay ra "MÁY KHÁC"/null ở
          //   CẢ HAI bản (có vá lẫn gỡ vá) — một ÂM TÍNH GIẢ của thiết bị đo, không phải của mã.
          const tuoi = await page.evaluate(({ manTid, id }) => {
            const w = window;
            const canvas = document.querySelector(`[data-testid="${manTid}"] canvas`); const cv = canvas.getBoundingClientRect();
            const t = w.__demTuongTac?.tamMay?.(id); if (!t || !t.trongKhung) return null;
            ...
            const n = hopNhan.find((v) => v.machineId !== id && t.x >= v.hop.trai && t.x <= v.hop.phai && t.y >= v.hop.tren && t.y <= v.hop.duoi);
            return { X, Y, conBiPhu: n ? n.machineId : null, biChe: t.biChe ?? null };
          }, { manTid: man, id: bam2.id });
          if (!tuoi) { kq.bamNhieu.push({ may: bam2.id, boQua: "tam khong bam duoc o khung hien tai" }); continue; }
          if (tuoi.conBiPhu === null) { kq.bamNhieu.push({ may: bam2.id, boQua: "khung hien tai khong con bi nhan may khac de", biChe: tuoi.biChe }); continue; }
          bam2.X = tuoi.X; bam2.Y = tuoi.Y; bam2.nhan = [`${tuoi.conBiPhu}:`];
          await page.mouse.move(bam2.X, bam2.Y, { steps: 4 }); await page.waitForTimeout(150); await page.mouse.click(bam2.X, bam2.Y);
          await page.waitForURL(/\/twin\/may\/\d+/, { timeout: 3_000 }).catch(() => {}); await page.waitForTimeout(400);
          const d2 = (page.url().match(/\/twin\/may\/(\d+)/) ?? [])[1] ?? null;
          const idN = Number(bam2.nhan[0].split(":")[0]);
          kq.bamNhieu.push({ may: bam2.id, ma: tenMay[bam2.id] ?? null, nhanPhu: bam2.nhan, dich: d2 ? Number(d2) : null, dungMay: Number(d2) === bam2.id, laMayCuaNhan: Number(d2) === idN });
          console.log(`      BẤM tâm máy ${bam2.id} (${tenMay[bam2.id] ?? "?"}) bị nhãn ${bam2.nhan.join("|")} phủ ⇒ /twin/may/${d2} ⇒ ${Number(d2) === bam2.id ? "ĐÚNG MÁY" : Number(d2) === idN ? "SAI: MÁY CỦA NHÃN" : "SAI: MÁY KHÁC"}`);
          if (page.url().includes("/twin/may/")) { await page.goBack({ waitUntil: "domcontentloaded" }); await page.waitForTimeout(1_800); }
        }
        if (kq.sauKeo.biNhanKhacPhu.length === 0) console.log("      (0 máy bấm được bị nhãn máy khác phủ sau kéo)");
        const bam = null;
        if (bam) {
