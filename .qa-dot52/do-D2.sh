#!/bin/sh
# ĐỢT 52 · D-2 (MANG=chan) — K1–K11 (.qa-dot44/do33.mjs) + E1–E9 (.qa-dot44/do35.mjs) + I1–I6 (.qa-dot44/do36.mjs) + P3–P7 trên 3052, tuần tự (G97).
# Đầu ra .qa-dot52/$M/... (G65/G130). Khuôn = .qa-dot49/do-D2.sh, chỉ đổi cổng + thư mục. Hàng tạm có trap (G111).
cd /d/SOURCES/_twin_wt || exit 1
B=http://localhost:3052
M="${MANG:-chan}"; T=../.qa-dot52/$M
...
echo "--- E6: andon raised TAM may 14 ---"
...
echo "--- E9: msl now() TAM may 14 ---"
mkdir -p .qa-dot52/$M/E
node .qa-dot37/db.mjs chen-msl 14 > .qa-dot52/$M/E/e9-chen-msl.json; cat .qa-dot52/$M/E/e9-chen-msl.json; echo
MSL=$(node -e 'console.log(JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")).hang.id)' .qa-dot52/$M/E/e9-chen-msl.json)
node .qa-dot37/db.mjs msl-ts "$MSL" > .qa-dot52/$M/E/e9-msl-ts.json; cat .qa-dot52/$M/E/e9-msl-ts.json; echo
node .qa-dot35/db.mjs robot-ts > .qa-dot52/$M/E/e9-robot-db.json
node .qa-dot44/do35.mjs --role=A --vp=1600x900 --cases=e9 --tag=$T/E --base=$B --mang=$M 2>&1 | grep -E "$L"
node .qa-dot37/db.mjs xoa-msl "$MSL" > .qa-dot52/$M/E/e9-xoa-msl.json; cat .qa-dot52/$M/E/e9-xoa-msl.json; echo; MSL=""
...
echo "--- I6: user TAM chi analytics_oee ---"
...
echo "--- P3-P7 @1600 + @1280 ---"
for VP in 1600x900 1280x720; do node .qa-dot44/do38.mjs --cases=p3,p4,p5,p6,p7 --vp=$VP --tag=../.qa-dot52/$M/P --base=$B --mang=$M 2>&1 | grep -vE '^\s*$'; done
