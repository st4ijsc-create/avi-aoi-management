#!/usr/bin/env node
/**
 * Doc 32 Wave R2 (R2-B) — targeted apply of drizzle/0202_report_artifacts.sql
 ...
 * not re-run it. (Pattern: apply-migration-0201.mjs.)
 *
 *   node scripts/apply-migration-0202.mjs            # dev + test
 *   node scripts/apply-migration-0202.mjs --dev-only
 *   node scripts/apply-migration-0202.mjs --test-only
 */
import fs from "fs";
...
const MIGRATION_FILE = "0202_report_artifacts.sql";
...
const REQUIRED_COLS = ["id", "reportType", "format", "storageKey", "fileHash", "expiresAt", "createdBy", "source"];
...
async function applyTo(url, label) {
  ...
  try {
    ...
    // Verify: table + the load-bearing columns exist.
    ...
    if (missing.length > 0) {
      throw new Error(`verification failed — missing columns: ${missing.join(", ")}`);
    }
    ...
    console.log(`[0202] ${label}: applied + verified — report_artifacts (${present.size} cols)`);
  } finally {
    await sql.end();
  }
}
...
if (!devUrl) {
  console.error("[0202] DATABASE_URL not set (checked .env)");
  process.exit(1);
}
...
for (const [url, label] of targets) {
  try {
    ...
    if (label === "test" && /does not exist/i.test(String(err?.message))) {
      console.warn(`[0202] ${label}: database not found — run \`npm run test:db:setup\` first (it clones dev incl. 0202).`);
    } else {
      console.error(`[0202] ${label}: FAILED —`, err?.message ?? err);
      process.exitCode = 1;
    }
  }
}
