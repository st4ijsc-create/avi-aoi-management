#!/usr/bin/env node
/**
 * W8-A (doc 27 M12a/M13, doc 29 §1+§4) — targeted apply of
 * drizzle/0191_component_library_capabilities.sql to the dev DB (DATABASE_URL
 * from .env) AND the cloned test DB (<db>_test), recording the file in
 * __applied_migrations exactly like migrate-standalone so a later
 * `npm run db:push` does not re-run it.
 *
 *   node scripts/apply-migration-0191.mjs            # dev + test
 *   node scripts/apply-migration-0191.mjs --dev-only
 *   node scripts/apply-migration-0191.mjs --test-only
 */
import fs from "fs";
...
const MIGRATION_FILE = "0191_component_library_capabilities.sql";
...
async function applyTo(url, label) {
  ...
  try {
    ...
    const [seed] = await sql`
      SELECT count(*)::int AS cnt FROM component_packages WHERE "origin" = 'seed'`;
    if (tbl.cnt !== 2 || cols.cnt !== 4 || seed.cnt < 40) {
      throw new Error(`verification failed (tables=${tbl.cnt}/2, cols=${cols.cnt}/4, seed=${seed.cnt}/>=40)`);
    }
    const [linked] = await sql`
      SELECT count(*)::int AS cnt FROM materials WHERE "packageId" IS NOT NULL`;
    ...
    console.log(`[0191] ${label}: applied + verified (2 tables, 4 columns, ${seed.cnt} seed packages, ${linked.cnt} material(s) backfilled to a package)`);
  } finally {
    await sql.end();
  }
}
...
if (!devUrl) {
  console.error("[0191] DATABASE_URL not set (checked .env)");
  process.exit(1);
}
...
for (const [url, label] of targets) {
  try {
    ...
    if (label === "test" && /does not exist/i.test(String(err?.message))) {
      console.warn(`[0191] ${label}: database not found — run \`npm run test:db:setup\` first (it clones dev incl. 0191).`);
    } else {
      console.error(`[0191] ${label}: FAILED —`, err?.message ?? err);
      process.exitCode = 1;
    }
  }
}
