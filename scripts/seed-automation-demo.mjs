...
async function main() {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // 5) EQUIPMENT 3D MODELS — PLACEHOLDER glTF, conversionStatus='external' (HONEST)
  // ═══════════════════════════════════════════════════════════════════════════
  // Bind one to a real machine (if any) + one class-level fallback. The URI is a
  // PLACEHOLDER — no real asset is shipped; conversionStatus='external' records
  // that the asset lives outside this system (see twin.ts header honesty note).
  const modelDefs = [];
  if (machineRows[0]) {
    modelDefs.push({
      ...
      modelUri: "https://placeholder.invalid/models/aoi-machine.glb",
      notes: `PLACEHOLDER model for machine ${machineRows[0].code} — no real asset; external conversion pending.`,
    });
  }
  // Class-level fallback (used by resolve when a specific machine has no model).
  modelDefs.push({
    modelKey: "MODEL-CLASS-ROBOT",
    machineId: null,
    equipmentClass: "ROBOT",
    modelUri: "https://placeholder.invalid/models/robot-arm.glb",
    notes: "PLACEHOLDER class-level model for ROBOT — no real asset; external.",
  });
  for (const m of modelDefs) {
    await upsert(
      ...
      sql`INSERT INTO equipment_3d_models ("modelKey", "machineId", "equipmentClass", "modelUri", "modelKind", "sourceFormat", "conversionStatus", status, scope, notes, "corporateCode", "factoryId")
          VALUES (${m.modelKey}, ${m.machineId}, ${m.equipmentClass}, ${m.modelUri}, ${"gltf"}, ${"step"}, ${"external"}, ${"active"}, ${"demo"}, ${m.notes}, ${CORP}, ${FACTORY_ID})
          ON CONFLICT ("modelKey") DO NOTHING RETURNING id`,
